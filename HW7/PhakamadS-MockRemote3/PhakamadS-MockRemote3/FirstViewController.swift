...
class FirstViewController: UIViewController {
    ...
    var newFavChannels = [ConfigurationViewController.FavoriteChannelDetails()]
    ...
    // receive message from other view controller
    override func viewWillAppear(_ animated: Bool) {
        for new in newFavChannels {
            if let indexSegment = new.index,
                let nameSegment = new.name,
                let channelSegment = new.channel {
                favoriteChannelSegmented.selectedSegmentIndex = indexSegment
                favoriteChannelSegmented.setTitle(nameSegment, forSegmentAt: indexSegment)
                listOfFavorite[indexSegment] = channelSegment
            }
        }
    ...
    @IBAction func channelMinus(_ sender: UIButton) {
        // if number is 0 then can't go minus
        if channelLabel.text! == "0" {
            channelLabel.text! = "0"
            
        // if number is 01 then can't go minus
        } else if Int(channelLabel.text!)! == 01 {
